...
const StyledGlassCard = styled.div`
  ...
  transform: rotate(${(props) => (props.$isHovered ? "0deg" : props.rotation + "deg")});
  cursor: pointer;
  overflow: hidden;
    ...
    >
      {cards.map((card, index) => (
        <StyledGlassCard
          ...
          $isHovered={isHovered}
          data-text={card.text}
          onClick={() => console.log(`${card.text} clicked`)}
...
// Alternative version with individual card hover states
export const GlassCardV2 = ({ cards }) => {
  ...
  return (
    <Container className="glass-cards-container">
      {displayCards.map((card, index) => (
        <StyledGlassCard
          key={index}
          rotation={card.rotation}
          $isHovered={hoveredCard === index || hoveredCard !== null}
          data-text={card.text}
